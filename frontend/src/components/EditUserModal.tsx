      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-scaleUp transition-colors">
        ...
        <div className="bg-slate-100 dark:bg-gradient-to-r dark:from-slate-800 dark:to-indigo-950 px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-base font-bold text-slate-900 dark:text-white">Editar Perfil de Usuario</h2>
              <p className="text-xs text-slate-500 dark:text-slate-400">Actualizar información personal y credenciales</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="bg-slate-50 dark:bg-slate-800/60 p-3 rounded-xl border border-slate-200 dark:border-slate-700/80 flex items-center justify-between">
            <div>
              <span className="text-[11px] text-slate-500 dark:text-slate-400 block">Cuenta de Acceso</span>
              <span className="text-xs font-mono font-medium text-slate-800 dark:text-slate-200">
                {user.credencial_acceso}
              </span>
            </div>
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-indigo-500/10 dark:bg-indigo-500/20 border border-indigo-500/30 text-indigo-600 dark:text-indigo-300 text-xs font-bold">
          ...
          <div className="border-t border-slate-200 dark:border-slate-800 pt-3">
            <span className="block text-xs font-semibold text-slate-700 dark:text-slate-400 mb-2 flex items-center gap-1.5">
            ...
            <div className="space-y-3">
              <div>
                <label className="block text-[11px] text-slate-500 dark:text-slate-400 mb-1">Nueva Contraseña</label>
                <input
                  type="password"
                  value={password}
